...
def prj_env_update(request, prj_env_id):
    if request.method == 'POST':
        ...
        access_key = request.POST.get('access_key')
        description = request.POST.get('description')
        ProjectEnv.objects.filter(id=prj_env_id).update(env=env, project=project, sign=sign, url=url,
                                                        access_id=access_id,
                                                        access_key=access_key)
        return HttpResponseRedirect('/prj_env/')
    ...
    prj_list = Project.objects.all()
    sign_list = Sign.objects.all()
    return render(request, "prj_env_update.html",
                  {"prj_env": prj_env, 'prj_list': prj_list, 'env_list': env_list, 'sign_list': sign_list})
    ...
    if request.method == 'POST':
        ...
        for step in steps:
            ...
            if step_id:
                ...
                Step.objects.filter(id=step_id).update(sn=step.get('sn'), data=step.get('data'),
                                                       assertion=step.get('assertion'))
            else:
                new_step = Step(case=case, sn=step.get('sn'), api=api, data=step.get('data'),
                                assertion=step.get('assertion'))
                new_step.save()
...
def case_add(request):
    ...
    return render(request, "case_add.html",
                  {'api_list': api_list, 'prj_list': prj_list, 'plan_list': plan_list, 'env_list': env_list})
...
def case_update(request, case_id):
    ...
    env_list = Environment.objects.all()
    prj_list = Project.objects.all()
    return render(request, "case_update.html",
                  {"case": case, 'api_list': api_list, 'plan_list': plan_list, 'step_list': step_list,
                   'env_list': env_list, 'prj_list': prj_list})
...
def plan_run(request):
    ...
    for case in cases:
        ...
        for step in case.step_set.all():
            ...
            case_result["steps"].append(
                {"api_name": step_api.name, "api_url": url, "api_data": data, "api_response": api_response,
                 "api_result": status})
            step_result = StepResult(test_case_result=test_case_result, step=step, api_name=step_api.name, api_url=url,
                                     api_data=data, api_response=api_response, status=status)
            step_result.save()
    ...
    TestReport.objects.filter(id=test_report.id).update(content=json.dumps(plan_result), case_num=case_num,
...
def report_detail(request, report_id):
    if request.method == 'GET':
        ...
        return render(request, "report_detail.html",
                      {"report": report, 'case_result_list': case_result_list, 'step_result_list': step_result_list})
